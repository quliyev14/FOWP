...
namespace ForOfficialWorkProject.DB
{
    public static class DB
    {
        ...
        public static void ProductWriteLog(in string log, IEnumerable<Product> objects)
        {
            lock (_sync)
                WriteLog(log, objects); 
        }
        ...
        public static void JsonWrite(string path, in string log, IEnumerable<Product> objects)
        { 
            var jsonOptions = new JsonSerializerOptions() { WriteIndented = true };
            if (!File.Exists(path))
            {
                File.WriteAllText(path, JsonSerializer.Serialize(objects, jsonOptions));
                objects = JsonRead<Product>(path)!;
            }
            File.WriteAllText(path, JsonSerializer.Serialize(objects, jsonOptions));
